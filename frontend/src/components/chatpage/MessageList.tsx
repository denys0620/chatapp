...
const MessageList: React.FC<MessageListProps> = ({ messages }) => {
  const messagesEndRef = useRef<HTMLDivElement>(null); // ✅ moved here
      ...
      {messages.map((msg, index) => (
        ...
        >
          ...
          <div className="flex-shrink-0 relative">
            <div
              className={`w-12 h-12 rounded-full ${getAvatarGradient(
                msg.user.username
              )} flex items-center justify-center text-white text-sm font-bold shadow-lg group-hover:shadow-xl transition-all duration-300 ring-2 ring-white dark:ring-gray-700`}
            >
